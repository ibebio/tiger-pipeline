rule collect_allelic_counts_f2:
    input:
        ...
        marker_complete_ref_vcf=lambda wildcards: "results/variants/parental/biallelic_snps_complete/{parent_ref}.vcf".format(parent_ref=[c["parent_ref"] for c in config["crossings"] if c["id"] == wildcards.crossing_id][0])
    output:
