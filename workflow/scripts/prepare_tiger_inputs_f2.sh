...
awk '{if ($1 !="Pt" && $1!="Mt") print $1 "\t" $2 "\t" $3 "\t" $6 "\t" $4 "\t" $5}' ${TAB_FILE} > ${WORKDIR}/${SAMPLE_NAME}.input.temp
...
cat ${WORKDIR}/${SAMPLE_NAME}.input.complete.temp | awk '{ if (($4 != ".") && ($4 ~ /^[0-9]*$/) && (length($3)==1) && (length($5)==1)) { print $0}}' | sed 's/\./0/g' > ${COMPLETE_FILE}
